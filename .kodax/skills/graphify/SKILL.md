---
name: graphify
description: any input (code, docs, papers, images) → knowledge graph + record store + health report + service-agent assets
trigger: /graphify
context: fork
---
...
Turn any folder of files into a persistent knowledge base: knowledge graph, structured record store, health checks, and service-agent assets. HTML remains optional; the primary output is a corpus another user or agent can query without rereading raw files.
...
graphify is built around Andrej Karpathy's /raw folder workflow: drop anything into a folder - papers, tweets, screenshots, code, notes - and get a compiled knowledge base with graph structure, row-level records, evidence trails, and reusable agent outputs.

Four things it does that Claude alone cannot:
...
4. **Structured service layer** - explicit assets under `records/`, `graph/`, `reports/`, `memory/`, and `health/` let other agents consume facts without parsing the raw corpus again.
...
Tabular/statistical/operational files (including normalized `.md` sidecars generated from pdf/docx/xlsx/csv/tsv/txt):
  preserve full record granularity. If the file contains a table or time series with <= 1000 rows, emit one node per explicit row/observation, not just summaries.
  Do not collapse row-level data into min/max/range-only abstractions when the source contains full records.
  For row nodes, include the key fields directly in the node label and preserve the full row in optional node metadata such as `record_json` or `record_key`.
  Emit summary rows and aggregate statistics as separate nodes in addition to row nodes.
...
If frontmatter contains `original_source_file`, use that value as `source_file` on emitted nodes and edges instead of the normalized sidecar path.
...
### Step 9 - Save manifest, build knowledge assets, update cost tracker, clean up, and report
...
# Build explicit knowledge assets for service-agent workflows
asset_summary = build_knowledge_assets(Path('.'), detection_result=detect)

print(f'This run: {input_tok:,} input tokens, {output_tok:,} output tokens')
print(f'All time: {cost[\"total_input_tokens\"]:,} input, {cost[\"total_output_tokens\"]:,} output ({len(cost[\"runs\"])} runs)')
print(f'Record store: {asset_summary[\"records\"]}')
print(f'Health report: {asset_summary[\"health_markdown\"]}')
"
...
Graph complete. Outputs in PATH_TO_DIR/graphify-out/
  ...
  records/records.jsonl - structured records for agent consumption
  reports/GRAPH_REPORT.md - copied report asset
  health/HEALTH_REPORT.md - extraction + coverage health checks
  memory/               - write-back knowledge from Q&A
...
For service-agent workflows, prefer:
...
When executed via the KnowledgeEngineering Demo backend:

- **Working directory**: The execution CWD is the `graphify-workspace/` directory. Files are pre-placed in `raw/` by the backend before this skill runs.
- **Python interpreter**: Use the value from `GRAPHIFY_PYTHON` environment variable if set, otherwise fall back to the standard detection logic in Step 1.
- **Step 3B - Semantic extraction without Agent tool**: Since this runtime does not have an Agent/Task tool for parallel subagent dispatch, use the provided `semantic_extract_helper.py` script instead. Run it via bash:
  ```bash
  $(cat .graphify_python) semantic_extract_helper.py
  ```
  This script reads `.graphify_uncached.txt`, splits files into chunks, calls the Gateway API (`http://127.0.0.1:3011/api/gateway/v1/skills/run` with skill `graphify-semantic-extract`) in parallel using `concurrent.futures.ThreadPoolExecutor`, and writes results to `.graphify_semantic_new.json`. If the script is not found, fall back to reading each file and extracting entities/relationships yourself sequentially via bash + Python.
- **Query mode**: When invoked with `/graphify query "..."`, also read the relevant `graphify-out/converted/*.md` files for direct evidence. The converted markdown files contain the original document content and should be used as primary evidence alongside graph traversal results.
- **Output format**: For query results, structure your final answer clearly using Markdown. The answer text will be extracted and displayed to the user.
