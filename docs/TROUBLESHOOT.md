...
## 10. PDF 表格数据拼接错误（396 → 3968）

**现象**

文档中 8 点的耗气量为 `396`，但知识检索返回 `3968`。实体属性中也存储了错误值。

**原因**

`pdf-parse` 库只提取 PDF 文本流，不理解表格列结构。表格单元格的值被拼接成连续字符串：
```
原始: | 04:00 | 25.7 | 116.1 | ... | 396 | 89.8 |
pdf-parse: 04:0025.7116.1...39689.8
```
LLM 从拼接字符串中推测数字边界，将 `396` 和 `89.8` 误读为 `3968` 和 `9.8`。

**解决方案**

用 Docling（IBM 开源 TableFormer 深度学习模型）替代 pdf-parse：
1. Docker 部署 `docling-serve`：`docker run -d --name docling-serve -p 5001:5001 ghcr.io/docling-project/docling-serve-cpu:latest`
2. 后端通过 `/v1/convert/file` multipart 上传 PDF
3. Docling 返回结构化 Markdown，表格以 `| col | col |` 格式保留，不存在拼接问题
4. 新增 `DocumentParserService` 统一格式路由（PDF → Docling，Excel → exceljs，Word → mammoth）
5. 移除 `pdf-parse` 及 `preprocessTableText` / `tokenizeNumbers` 启发式代码

**涉及文件**

- `backend/src/services/DocumentParserService.ts`（新建）
- `backend/src/skills/documentParse.ts`（重写）
- `backend/package.json`（移除 pdf-parse，新增 exceljs/mammoth/pdf2pic）

---

## 11. MiniMax-VL 视觉通道不可用

**现象**

双通道模式下视觉通道报错 `your current token plan not support model, MiniMax-Text-01` 或 `invalid api key`。系统降级为 Docling 单通道。

**原因**

MiniMax API 套餐不包含 `MiniMax-Text-01` 视觉模型。改用 `MiniMax-M2.7` 后虽不报错，但视觉表格提取能力弱（每页仅返回 24-88 字符，无表格）。

**解决方案**

默认关闭视觉通道，仅走 Docling 单通道（精度 97.9%，已足够）。如需开启：
1. `backend/.env` 添加 `ENABLE_VISION_CHANNEL=true`
2. 确保 MiniMax 套餐支持视觉模型，或替换为其他 VLM（GPT-4V、Claude Vision）
3. 修改 `VisionExtractService.ts` 中的 `VISION_MODEL` 和 `MINIMAX_VL_ENDPOINTS`

**涉及文件**

- `backend/src/services/VisionExtractService.ts`
- `backend/src/skills/documentParse.ts`（`ENABLE_VISION_CHANNEL` 开关）
...
| PDF 表格数值错误（如 396→3968）| 检查 Docling Docker 容器是否运行：`docker ps \| grep docling` |
| PDF 上传后解析失败 | 检查 Docling 服务是否可达：`curl http://localhost:5001/health` |
| Docling 容器未启动 | `docker start docling-serve` 或 `docker run -d --name docling-serve -p 5001:5001 ghcr.io/docling-project/docling-serve-cpu:latest` |
| 视觉通道报 model not supported | 当前套餐不含视觉模型，默认已关闭视觉通道，无需处理 |
