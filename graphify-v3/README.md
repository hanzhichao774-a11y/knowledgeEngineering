...
**An AI coding assistant skill and local knowledge-base compiler.** Type `/graphify` in Claude Code, Codex, OpenCode, OpenClaw, or Factory Droid - it reads your files, normalizes messy sources, builds a knowledge graph plus structured record store, and gives you back service-agent-ready assets. Understand a codebase faster. Preserve row-level operational facts. Find the "why" behind architectural decisions.
...
> Andrej Karpathy keeps a `/raw` folder where he drops papers, tweets, screenshots, and notes. graphify is the answer to that problem - not just a graph, but a compiled local knowledge base with reusable records, health checks, and agent-facing outputs.
...
```
graphify-out/
├── graph.html               interactive graph (optional)
├── GRAPH_REPORT.md          top-level audit report
├── graph.json               persistent graph - query weeks later without re-reading
├── assets.json              manifest of compiled knowledge assets
├── raw/source_index.json    source inventory
├── normalized/              normalized sidecar index (PDF/Office/table heavy inputs)
├── records/records.jsonl    row-level structured records for agent consumption
├── graph/graph.json         copied graph asset for service workflows
├── reports/GRAPH_REPORT.md  copied report asset
├── health/HEALTH_REPORT.md  coverage and confidence warnings
├── memory/                  write-back knowledge from Q&A and briefings
...
graphify now behaves like a small local knowledge-base system:

1. **Ingest** raw files from any folder.
2. **Normalize** messy formats into sidecars. PDF inputs can use a MinerU-first path when the `mineru` CLI is available, which improves OCR and image-table recovery before semantic extraction.
3. **Compile** graph nodes, edges, rationale, and row-level records from the normalized corpus.
4. **Index** explicit assets under `records/`, `graph/`, `reports/`, `memory/`, and `health/`.
5. **Serve** the compiled corpus through CLI and MCP tools for other agents.

The old graph pipeline still exists inside this flow: a deterministic AST pass extracts structure from code files (classes, functions, imports, call graphs, docstrings, rationale comments) with no LLM needed. Parallel subagents then process docs, papers, and images. The results are merged into a NetworkX graph, clustered with Leiden community detection, and exported as HTML, JSON, a report, and service-agent assets.
...
Every relationship is tagged `EXTRACTED` (found directly in source), `INFERRED` (reasonable inference, with a confidence score), or `AMBIGUOUS` (flagged for review). You always know what was found vs guessed, and the health report makes low-confidence coverage gaps explicit.
...
/graphify ./raw --mcp              # start MCP stdio server with graph + record tools

# service-agent commands
graphify agent "北京市政热网水耗排名从低到高分别是什么？"
graphify agent "海淀分公司诉求总量" --format structured
...
| Papers | `.pdf` | MinerU-first normalization when available, then citation mining + concept extraction |
| Images | `.png .jpg .webp .gif` | Claude vision - screenshots, diagrams, any language |

## Agent-first outputs

The graph is no longer the only product. `graphify` also writes explicit knowledge assets that other users or agents can consume directly:

- `records/records.jsonl`: row-level facts extracted from tables, time series, and operational reports
- `graph/graph.json`: graph asset for service and MCP workflows
- `reports/GRAPH_REPORT.md`: markdown briefing
- `health/HEALTH_REPORT.md`: quality checks for coverage, MinerU usage, ambiguous edges, and empty memory
- `memory/`: write-back space for derived briefings and later compounding

The built-in service layer exposes:

- `graphify agent "<question>" --format direct`
- `graphify agent "<question>" --format structured`
- `graphify health`
- MCP tools: `ask_kb`, `search_records`, `get_health`
