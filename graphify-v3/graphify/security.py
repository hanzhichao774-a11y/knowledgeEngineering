...
def validate_url(url: str) -> str:
    ...
    for SSRF or local file access. Also blocks requests to private IP ranges
    (127.x, 10.x, 169.254.x, etc.) and cloud metadata endpoints to prevent
    ...
    if hostname:
        ...
        # Block literal IP hosts aggressively, including reserved ranges.
        try:
        ...
        if literal_ip is not None:
            if (
                _is_blocked_ip(literal_ip)
            ):
                raise ValueError(
                    f"Blocked private/internal IP {literal_ip} (from literal host '{hostname}'). "
                    f"Got: {url!r}"
                )
        else:
            # For public hostnames, resolve and block clear local-network SSRF targets.
            # Do not treat DNS-resolved reserved ranges as fatal because some corporate
            # DNS and captive-network environments alias public domains into reserved
            # benchmark ranges, which would otherwise break legitimate URLs.
            try:
