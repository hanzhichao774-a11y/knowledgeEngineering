...
def main() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help"):
        ...
        print("  agent \"<question>\"      query the knowledge base service layer")
        print("    --format F             direct|evidence|structured|artifact (default direct)")
        print("    --root <path>          corpus root (default .)")
        print("    --graph <path>         path to graph.json (default graphify-out/graph.json)")
        print("  assets build            build knowledge asset indexes under graphify-out/")
        print("    --root <path>          corpus root (default .)")
        print("    --graph <path>         graph source (default graphify-out/graph.json)")
        print("    --report <path>        report source (default graphify-out/GRAPH_REPORT.md)")
        print("  health                  print the current health report")
        print("    --root <path>          corpus root (default .)")
    ...
    if cmd == "install":
        ...
        if len(sys.argv) < 3:
            ...
            sys.exit(1)
        from graphify.serve import _score_nodes, _bfs, _dfs, _subgraph_to_text, tokenize_query
        ...
        if len(sys.argv) < 3:
            print("Usage: graphify agent \"<question>\" [--format direct|evidence|structured|artifact] [--root path] [--graph path]", file=sys.stderr)
            sys.exit(1)
        ...
        while i < len(args):
            if args[i] == "--format" and i + 1 < len(args):
                ...
                output_format = args[i].split("=", 1)[1]; i += 1
            elif args[i] == "--root" and i + 1 < len(args):
                root = Path(args[i + 1]); i += 2
            elif args[i].startswith("--root="):
                root = Path(args[i].split("=", 1)[1]); i += 1
            elif args[i] == "--graph" and i + 1 < len(args):
                graph_path = args[i + 1]; i += 2
                ...
                i += 1
        print(answer(question, root, output_format=output_format, graph_path=graph_path))
    elif cmd == "assets":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd != "build":
            print("Usage: graphify assets build [--root path] [--graph path] [--report path]", file=sys.stderr)
            sys.exit(1)
        ...
        while i < len(args):
            if args[i] == "--root" and i + 1 < len(args):
                ...
                graph_path = Path(args[i].split("=", 1)[1]); i += 1
            elif args[i] == "--report" and i + 1 < len(args):
                report_path = Path(args[i + 1]); i += 2
            elif args[i].startswith("--report="):
                report_path = Path(args[i].split("=", 1)[1]); i += 1
            else:
                i += 1
        summary = build_knowledge_assets(root, graph_path=graph_path, report_path=report_path)
        ...
        while i < len(args):
            if args[i] == "--root" and i + 1 < len(args):
                ...
                i += 1
        health_path = root / "graphify-out" / "health" / "HEALTH_REPORT.md"
        if not health_path.exists():
            from graphify.assets import build_knowledge_assets
            build_knowledge_assets(root)
        if not health_path.exists():
            print("No health report found.", file=sys.stderr)
            sys.exit(1)
