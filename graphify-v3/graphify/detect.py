...
from .normalize import convert_delimited_file, convert_pdf_file, convert_structured_text_file
...
DOC_EXTENSIONS = {'.md', '.txt', '.rst', '.csv', '.tsv'}
    ...
    for p in all_files:
        ...
        if not in_memory:
            ...
            if p.name.startswith("."):
                continue
            # Skip graphify's own outputs except memory artifacts explicitly written
            # for later re-ingestion.
            if str(p).startswith(str(graphify_out_dir)):
                continue
        ...
        if ftype:
            ext = p.suffix.lower()
            # Office, delimited, structured text, and PDF files: convert to markdown
            # sidecars so subagents can preserve row-level records instead of only summaries.
            if ext in OFFICE_EXTENSIONS:
                ...
                continue
            if ext in {".csv", ".tsv"}:
                md_path = convert_delimited_file(p, converted_dir, delimiter="\t" if ext == ".tsv" else ",")
                if md_path:
                    files[ftype].append(str(md_path))
                    total_words += count_words(md_path)
                    continue
            if ext == ".pdf":
                md_path, image_paths = convert_pdf_file(p, converted_dir)
                if md_path:
                    files[ftype].append(str(md_path))
                    total_words += count_words(md_path)
                for image_path in image_paths:
                    files[FileType.IMAGE].append(str(image_path))
                if md_path or image_paths:
                    continue
            if ext == ".txt":
                md_path = convert_structured_text_file(p, converted_dir)
                if md_path:
                    files[ftype].append(str(md_path))
                    total_words += count_words(md_path)
                    continue
