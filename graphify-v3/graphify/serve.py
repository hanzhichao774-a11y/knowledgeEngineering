...
from graphify.agent import answer as answer_kb, search_records as kb_search_records
...
def _knowledge_root_from_graph_path(graph_path: str) -> Path:
    safe = validate_graph_path(graph_path)
    if safe.parent.name == "graphify-out":
        return safe.parent.parent
    if safe.parent.parent.name == "graphify-out":
        return safe.parent.parent.parent
    return safe.parent
...
def tokenize_query(question: str) -> list[str]:
    """Tokenize Latin and CJK queries for fuzzy label matching."""
...
def serve(graph_path: str = "graphify-out/graph.json") -> None:
    ...
    knowledge_root = _knowledge_root_from_graph_path(graph_path)
    ...
    async def list_tools() -> list[types.Tool]:
        return [
            ...
            types.Tool(
                name="ask_kb",
                description="Query the knowledge base as a service agent. Returns direct, evidence, structured, or artifact output.",
                inputSchema={
                    "type": "object",
                    "properties": {
                        ...
                        "format": {
                            "type": "string",
                            "enum": ["direct", "evidence", "structured", "artifact"],
                            "default": "direct",
                        },
                        "top_n": {"type": "integer", "default": 5, "description": "Maximum evidence items to return"},
                    },
                    "required": ["question"],
                },
            ),
            types.Tool(
                name="search_records",
                description="Search the structured record store extracted from tables, statistics, and time-series data.",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "question": {"type": "string", "description": "Question or keywords to search in records"},
            ...
            types.Tool(
                name="get_health",
                description="Return the current health report for the knowledge base, including missing coverage and low-confidence warnings.",
                inputSchema={"type": "object", "properties": {}},
            ),
        ]
    ...
    def _tool_ask_kb(arguments: dict) -> str:
        return answer_kb(
            arguments["question"],
            knowledge_root,
            output_format=arguments.get("format", "direct"),
            graph_path=str(validate_graph_path(graph_path)),
            top_n=int(arguments.get("top_n", 5)),
        )

    def _tool_search_records(arguments: dict) -> str:
        matches = kb_search_records(arguments["question"], knowledge_root, top_n=int(arguments.get("top_n", 10)))
        if not matches:
            return "No structured records matched the query."
        return json.dumps(matches, ensure_ascii=False, indent=2)

    def _tool_get_health(_: dict) -> str:
        health_path = knowledge_root / "graphify-out" / "health" / "health.json"
        if not health_path.exists():
            return "No health report found. Run `graphify assets build` or the updated /graphify pipeline first."
