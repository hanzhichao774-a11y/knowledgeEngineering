...
def test_detect_routes_pdf_page_images_to_image_bucket(tmp_path, monkeypatch):
    pdf_path = tmp_path / "scan.pdf"
    pdf_path.write_bytes(b"%PDF-1.4 test")

    converted_dir = tmp_path / "graphify-out" / "converted"
    converted_dir.mkdir(parents=True, exist_ok=True)
    md_path = converted_dir / "scan_fake.md"
    md_path.write_text("# normalized\n", encoding="utf-8")
    image_path = converted_dir / "scan_fake_images" / "scan_page021_img1.jpg"
    image_path.parent.mkdir(parents=True, exist_ok=True)
    image_path.write_bytes(b"\xff\xd8\xff\xd9")

    monkeypatch.setattr("graphify.detect.convert_pdf_file", lambda path, out_dir: (md_path, [image_path]))

    result = detect(tmp_path)

    assert str(md_path) in result["files"]["paper"]
    assert str(image_path) in result["files"]["image"]
