...
- 如果 docSnippets 中有相关数据但 graphifyAnswer 说没有，以 docSnippets 为准
- 如果所有数据源都完全没有相关内容，友好说明知识库中暂未收录相关知识
- 回答使用中文

数据源优先级说明：
- docSnippets 是从原始文档中直接提取的文本片段，包含真实数据，是最可靠的数据来源
- graphifyAnswer 是从知识图谱中生成的回答
- recordResults 是从知识图谱记录中检索的结果
- 如果 docSnippets 包含用户询问的数据，即使 graphifyAnswer 未提及，也必须使用 docSnippets 中的数据`;

export const GRAPHIFY_SEMANTIC_EXTRACT_SYSTEM_PROMPT = `你是一个高级知识工程师。从提供的文档内容中提取知识图谱的节点（entities/concepts）和关系（edges），用于构建 NetworkX 知识图谱。

## 提取要求

### 节点（nodes）
每个节点需要：
- id: 全局唯一标识符，格式 "entity::名称" 或 "concept::名称"
- label: 简洁的中文名称
- type: 节点类型（entity/concept/event/metric/organization/person/location/time/document）
- source_file: 来源文件名
- confidence: 置信度 0.0-1.0（明确出现为 1.0，推断关系为 0.6-0.8）
...
- nodes: 参与的节点 id 列表
- label: 关系名称
- type: "co-occurrence" / "process" / "group"

## 提取策略
1. 表格数据：每行提取为独立实体，列标题作为属性关系
2. 时间序列：时间点作为 time 类型节点，与指标形成 "记录" 关系
3. 层级结构：提取 "包含" / "属于" 关系
4. 因果关系：提取 "导致" / "影响" 关系
5. 数值数据：保留原始数值（含单位）作为节点 description
...
export const GRAPHIFY_COMMUNITY_LABEL_SYSTEM_PROMPT = `你是一个知识图谱分析专家。为知识图谱的社区（聚类）生成简洁的中文标签名称。
...
- 标签应反映社区成员的共同主题
- 不要使用"社区"、"分组"等元词汇
- 如果成员主要是某个领域的概念，用该领域名称

输出严格 JSON（不要 markdown 代码块），key 为 communityId，value 为标签：
{
  "0": "供热设备管理",
  "1": "运行数据监测"
}`;
